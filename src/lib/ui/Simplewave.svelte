<style>
    h1 {
        font-weight:300;
        letter-spacing: 2px;
        font-size:48px;
    }

    p {
        letter-spacing: 1px;
        color: #363636;
        font-size: 1em;
    }

    .header {
        position:relative;
        text-align:center;
        color:white;
    }
    

    .inner-header {
        height:75vh;
        width:100%;
        margin: 0;
        padding: 0;
    }

    .flex { /*Flexbox for containers*/
        display: flex;
        justify-content: center;
        align-items: center;
        text-align: center;
    }

    .waves {
        position:relative;
        width: 100%;
        height:15vh;
        margin-bottom:-7px; /*Fix for safari gap*/
        min-height:100px;
        max-height:150px;
    }

    .content {

        height:20vh;
        text-align:center;
        
        background-color: white;
        position: absolute;
        width: 100%;
        
        bottom: 0;

    }


    /* Animation */

    .parallax > use {
     
        animation: move-forever 25s cubic-bezier(.55,.5,.45,.5)     infinite;
    }

    .parallax > use:nth-child(1) {
        animation-delay: -2s;
        animation-duration: 7s;
    }
    .parallax > use:nth-child(2) {
        animation-delay: -3s;
        animation-duration: 10s;
    }

    .parallax > use:nth-child(3) {
        animation-delay: -4s;
        animation-duration: 13s;
    }
    .parallax > use:nth-child(4) {
    animation-delay: -5s;
    animation-duration: 20s;
    }
    
    @keyframes move-forever {
        0% {
        transform: translate3d(-90px,0,0);
        }
        100% { 
            transform: translate3d(85px,0,0);
        }
    }
    h2 {font-size:20px;}
    /*Shrinking for mobile*/
    @media (max-width: 768px) {
        .content {  height:20vh; }
        h1 { font-size:24px; }
        h2 {font-size:16px;}
    }
    .title_container {margin-bottom: 20vh;}
    .wave_container {
        position: absolute;
        width: 100%;
        top: 65vh;

    }
</style>

<div class="animated fadeIn slower">

    <div class="header">

        <!--Content before waves-->
        <div class="inner-header flex">
        
            <div class="container title_container">
                <slot name="header">
                    <h1 >FELion Spectrum Analyser</h1>
                    <br>
                    <h2 >To analyse FELIX data for FELion Instrument</h2>
                </slot>

                <slot name="buttonContainer"><!-- optional fallback --></slot>

            </div>
        </div>

        <!--Waves Container-->
        <div class="wave_container">
            <svg class="waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 24 150 28" preserveAspectRatio="none" shape-rendering="auto">
                <defs>

                    <path id="gentle-wave" d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z" />
                </defs>
                <g class="parallax">
                    <use xlink:href="#gentle-wave" x="48" y="0" fill="rgba(255,255,255,0.7" />
                    <use xlink:href="#gentle-wave" x="48" y="3" fill="rgba(255,255,255,0.5)" />
                    <use xlink:href="#gentle-wave" x="48" y="5" fill="rgba(255,255,255,0.3)" />
                    <use xlink:href="#gentle-wave" x="48" y="7" fill="#fff" />
                </g>

            </svg>
        </div>
        <!--Waves end-->

    </div>
    <!--Header ends-->
    
    <!--Content starts-->

    <div class="content flex">
        <slot name="footer"><p>Developed at Dr.Br&uuml;nken's group FELion@FELIX | 2021 &copy; AN Marimuthu | MIT License</p></slot>
    </div>

    <!--Content ends-->

</div>